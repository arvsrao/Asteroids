...
Game::Game(const int player_increment, const int rotation_increment):
        GameObject(),
        _randomNumberBetweenZeroAndTwo{RandomNumberBetween(1, 3)},
...
void Game::spawn(Renderer& renderer)
{
    ...
    while(_running) {
        ...
        if (timeDelta.count() > waitTime) {
            ...
            _futures.emplace_back(std::async(&Asteroid::checkForCollision, _asteroids.back(), _player, std::ref(_blasts)));

            waitTime = _randomNumberBetweenZeroAndTwo();
...
void Game::detectPlayerEnemyCollision() {
    ...
    while (_running) {
       ...
       std::lock_guard<std::mutex> lock(_mutex);
        ...
        for (size_t idx = 0; idx < _futures.size(); idx++) {
            if (_futures.front().valid()) {
                _explosions.emplace_back(_futures.front().get());
                _futures.pop_front();
            }
        }
...
void Game::renderAsteroids(Renderer &renderer, SDL_Texture *texture) {
    ...
    for (size_t idx = 0; idx < _asteroids.size(); idx++) {
        ...
        rock->move();
        if (!renderer.outsideScreen(*rock) && !rock->isHit()) {
...
void Game::renderExplosions(Renderer& renderer, SDL_Texture* texture) {
    ...
    for (size_t idx = 0; idx < _explosions.size(); idx++) {
        if (!_explosions.front().isFrameCountPositive()) {
            renderer.renderTexture(texture, _explosions.front());
            _explosions.pop_front();
        }
    }
}
...
void Game::run(Renderer& renderer, Controller& controller, int target_frame_duration) {
    ...
    // detect player - asteroid collisions on a separate thread.
    _threads.emplace_back(&Game::spawn, this, std::ref(renderer));
    _threads.emplace_back(&Game::detectPlayerEnemyCollision, this);
            ...
            for (size_t idx = 0; idx < _asteroids.size(); idx++) {
                ...
                for (size_t idy = 0; idy < _blasts.size(); idy++) {
                    ...
                    if (rock->collidesWith(*blast)) {
                        ...
                        _explosions.emplace_back(rock->getX(), rock->getY());
