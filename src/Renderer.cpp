...
void Renderer::clear() { SDL_RenderClear(_renderer); }

int Renderer::generateY() { return _randomY(); }

int Renderer::getScreenWidth() const { return SCREEN_WIDTH_; }
...
bool Renderer::outsideScreen(const RenderableEntity &entity) const {
  return entity.getX() < -50 || entity.getY() < -50 || entity.getX() > SCREEN_WIDTH_ ||
         entity.getY() > SCREEN_HEIGHT_ + 50;
}
...
SDL_Texture *Renderer::loadImage(const char *filename) const {
 ...
 */
void Renderer::renderTexture(SDL_Texture *texture, const RenderableEntity &entity) const {
  SDL_Rect dest;
  dest.x = entity.getX();
  dest.y = entity.getY();
  SDL_QueryTexture(texture, nullptr, nullptr, &dest.w, &dest.h);
  SDL_RenderCopyEx(_renderer, texture, nullptr, &dest, entity.getAngle(),
                   entity.getRotationCenter(), SDL_FLIP_NONE);
}

/**
 * Render a RenderableEntity according to its type. Each entity type has exactly
 * one texture.
 *
 * @param entity is a RenderableEntity
 */
void Renderer::renderTexture(const RenderableEntity &entity) const {
...
/**
 * Render the players current health in the upper right-hand corner as hearts.
 * @param health is an int representing the health score.
 */
...
void Renderer::init() {
  ...
  _window = SDL_CreateWindow("hello world!!", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
...
Renderer::Renderer(const int h, const int w)
    : SCREEN_WIDTH_(w),
      ...
      _randomY(RandomNumberBetween{0, h}) {
  init();
  _textures = Textures{loadImage("../resources/low-angle-shot-mesmerizing-starry-sky-klein.png"),
